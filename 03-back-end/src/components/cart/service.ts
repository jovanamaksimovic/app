...
export default class CartService extends BaseService<CartModel> {
    ...
    public async add(userId: number): Promise<CartModel|IErrorResponse> {
        return new Promise<CartModel|IErrorResponse>(async resolve => {
            ...
            .then(async res => {
                ...
                const newCartId = +(insertData?.inserId);
    ...
    public async getAllOrderByUserId(userId: number): Promise<CartModel[]> {
        const [rows] = await this.db.execute(
            `SELECT
                cart.*
            FROM
