...
import { IAddToCartValidator } from './dto/ICart';
import IAddToCart from './dto/ICart';
...
export default class CartController extends BaseController {
    private isCallerUser(req: Request, res: Response): boolean {
        if(req.authorized.role !== "user") {
            res.status(403).send("This action is only availbe to the user role");
            return false;
        }
    ...
    public async getAllOrdersForCurrentUser(req: Request, res: Response) {
        ...
        res.send(await this.services.cartService.getAllOrderByUserId(req.authorized?.id));
    }
    ...
    public async getAllOrdersByUserId(req: Request, res: Response) {
        ...
        res.send(await this.services.cartService.getAllOrderByUserId(userId));
    }
